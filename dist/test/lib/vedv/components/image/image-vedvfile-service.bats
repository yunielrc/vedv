...
@test "vedv::image_vedvfile_service::are_supported_commands() Should succeed With empty 'command'" {
  local -r commands=""

  run vedv::image_vedvfile_service::are_supported_commands "$commands"

  assert_success
  assert_output 'There are no commands for validation'
}

@test "vedv::image_vedvfile_service::are_supported_commands() Should fail With invalid command" {
...
@test "vedv::image_vedvfile_service::are_supported_commands() Should succeed With valid command" {
...
@test "vedv::image_vedvfile_service::validate_file() Should fail With nonexistent vedvfile" {

  run vedv::image_vedvfile_service::validate_file '/a/b/c/Vedvfile'

  assert_failure "$ERR_INVAL_ARG"
  assert_output "Invalid argument 'vedvfile', file /a/b/c/Vedvfile doesn't exist"
}

@test "vedv::image_vedvfile_service::validate_file() Should fail With unsupported commands" {

  local -r vedvfile_invalid='dist/test/lib/vedv/components/image/fixtures/Vedvfile-invalid'

  run vedv::image_vedvfile_service::validate_file "$vedvfile_invalid"
...
@test "vedv::image_vedvfile_service::validate_file() Should fail with file that does not meet hadolint validation" {
...
@test "vedv::image_vedvfile_service::validate_file() Should succeed" {
  local -r vedvfile='dist/test/lib/vedv/components/image/fixtures/Vedvfile'

  run vedv::image_vedvfile_service::validate_file "$vedvfile"
